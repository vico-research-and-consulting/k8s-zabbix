...
def get_pvc_data(api, node, timeout_seconds):
    ...
    for item in loaded_json["pods"]:
        ...
        for volume in item["volume"]:
            ...
            data = {"metadata": {"name": name, "namespace": namespace}, "item": volume}
            data["item"]["nodename"] = node

            data["item"]["usedBytesPercentage"] = (
                float(float(data["item"]["usedBytes"] / data["item"]["capacityBytes"])) * 100
            )

            data["item"]["inodesUsedPercentage"] = (
                float(float(data["item"]["inodesUsed"] / data["item"]["inodes"])) * 100
            )
...
class Pvc(K8sObject):
    ...
    def get_zabbix_metrics(self):
        ...
        for key, value in self.data["item"].items():
            data_to_send.append(
                ZabbixMetric(self.zabbix_host, f"check_kubernetesd[get,pvc,{self.name_space},{self.name},{key}]", value)
            )
