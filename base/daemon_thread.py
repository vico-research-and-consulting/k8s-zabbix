...
class CheckKubernetesDaemon:
    ...
    def __init__(
        ...
        config_name: str,
        resources: List[str],
        resources_excluded: List[str],
        resources_excluded_web: List[str],
    ...
    def start_data_threads(self):
        for resource in self.resources:
            ...
            if resource in ["containers", "services"]:
                ...
                thread = TimedThread(
                    resource, self.data_resend_interval, exit_flag, daemon=self, daemon_method="watch_data"
                )
                self.manage_threads.append(thread)
                thread.start()
            else:
                thread = WatcherThread(resource, exit_flag, daemon=self, daemon_method="watch_data")
                self.manage_threads.append(thread)
                thread.start()
    ...
    def start_api_info_threads(self):
        ...
        thread = TimedThread(
            "api_heartbeat", self.api_zabbix_interval, exit_flag, daemon=self, daemon_method="send_heartbeat_info"
        )
        self.manage_threads.append(thread)
        thread.start()
    ...
    def report_global_data_zabbix(self, resource):
        ...
        if resource == "services":
            ...
            data_to_send.append(
                ZabbixMetric(
                    self.zabbix_host, "check_kubernetes[get,services,num_ingress_services]", num_ingress_services
                )
            )
